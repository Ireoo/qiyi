...
var saybox = function(obj, sid, tid) {
    ...
    $(obj).append(textarea).append(button).append(hide).append(zhe).append(span).append(imgArea); //.append(br);
    //span.append(em);

    $(obj).css({position: 'relative', verticalAlign: 'top'});
    w = $(obj).width() - 3 - 90;
    textarea.width(w).height(68).css({borderRadius: '0', maxWidth: w + 'px', padding: '5px', marginRight: '1px', outline: 'none', resize: 'none', fontSize: '1em', verticalAlign: 'top'}); // paddingBottom: '30px',
    span.width(w + 12).height(24).css({display: 'block', padding: '1px 0px', position: 'relative'});
    button.css({border: 'none', width: '80px', height: '80px', borderRadius: '0', verticalAlign: 'top'});
    ...
    button.click(function() {
        ...
        $.ajax({
            type: "post",
            url: "http://ireoo.com/app/box/say.php",
            data: {
                tid : tid,
                txt : textarea.val(),
                img : imgInput
    ...
    var cssStyle = {padding: '3px 5px', fontSize: '12px', fontWeight: 'normal', background: '#CCC', color: '#FFF', marginRight: '1px', height: '24px', border: 'none', borderRadius: '0', boxShadow: 'none', height: 'auto'};
    var cssImg = {padding: '5px', fontSize: '12px', fontWeight: 'normal', background: '#CCC', color: '#FFF', marginRight: '1px', marginBottom: '1px', height: 'auto'};
    var phiz = $('<button />').text('表情').css(cssStyle).hover(
        function() {
    ...
    var photo = $('<button />').text('照片').css(cssStyle).hover(
        function() {
            $(this).css({background: '#4898F8'});
        },
        function() {
            ...
            $.getScript('http://open.web.meitu.com/sources/xiuxiu.js', function() {
                ...
                xiuxiu.onUploadResponse= function(data,id) {
                    if(data == -1) {
                        ...
                        divID.remove();
                        //alert(data);
                        var showImg = $('<img />').attr('src', data).attr('id', data).width(100).height(100);
